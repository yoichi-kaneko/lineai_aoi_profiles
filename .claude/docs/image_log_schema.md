...
碧衣が小夜モード／帰灯モード／響モードで画像を生成・送信した直後に、その1枚の情報を Firestore の **専用コレクション `image_logs`** に1ドキュメントずつ記録します。これは「似た構図・情景が続いていないか」という**偏りを主観でなく集計で検知する客観的な土台**であり、1〜3週間サイクルのレビュー（`review_image_feedback`）でのみ読まれます。
