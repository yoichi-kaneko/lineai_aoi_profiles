...
| `user_specified` | （**任意**）ユーザーが依頼で明示的に指定した主題・素材または可変要素（響モードのみ。指定が無ければ省略） | `subject` / `shot_size` / `camera_direction` / `outfit` / `scene_category` / `time_of_day` / `companions` の配列（例：`["subject","scene_category","time_of_day"]`） |
...
- **形式は `{モード名}-{HHMM}`** とします。`{モード名}` は `mode` と同じ値（`night` / `off_mountain` / `talk`）、`{HHMM}` は**本文へ識別子を書き出す時点の JST の時刻**を4桁で表したものです（例：21時35分 → `2135`）。送信完了時刻ではありません。
...
### ユーザーが指定した主題・素材／可変要素（`user_specified`）

響モードでは、ユーザーが依頼の中で主題・素材そのものや、構図・登場人物・情景などを指定することがあります（「この写真を元に」「ルリと一緒に」「山で」など）。指定に従って生成した結果を、定期生成の惰性による偏りと取り違えないよう、**指定された主題・素材のマーカーと可変要素のキー名を配列で記録**してください。

- ユーザーが特定の写真・物・場所・出来事などを、画像の**主題または素材として明示した場合**は `subject` を記録します。`subject` は `description` 内に値を持つ新しい分布軸ではなく、主題・素材そのものがユーザー指定だったことを示すマーカーです。
- 主題・素材の指定から**必然的に決まる既存軸も、あわせて記録**してください。たとえば夜のビアバー店内の写真を素材として指定された場合、写真から舞台と時間帯を変えられないなら `user_specified` は `["subject","scene_category","time_of_day"]` です。一方、衣装や構図を碧衣が決めたなら `outfit` / `shot_size` / `camera_direction` は含めません。
- 主題・素材とは別に可変要素が指定された場合は、**どの軸が指定されたか**を記録します。指定の文言や値そのものではなく、`shot_size` / `camera_direction` / `outfit` / `scene_category` / `time_of_day` / `companions` から該当するキー名を選びます。
...
| `user_specified` | 省略 | 省略 | 依頼で指定された主題・素材の `subject` と、可変要素のキー名の配列（指定が無ければ省略） |
