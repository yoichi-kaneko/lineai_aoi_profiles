...
> **なぜ専用コレクションか**: `notes` は `get_firestore_docs` が日付範囲だけで `type` 無差別に全件返すため、暁・望・小夜・帰灯・調べ・響の各モードが読み込みます。song_logs を `notes` に混ぜると日々の各モードのコンテキストへ無条件で流入し、トークンコスト増・ノイズ・誤取込の原因になります。読み手は調べモードの重複回避と将来の楽曲レビューだけなので、`song_logs` に隔離します。
