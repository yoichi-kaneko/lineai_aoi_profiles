...
1. プロンプトを、そのまま `tmp/mureka_lyrics_prompt.txt` に保存してください（改行はそのまま改行として書き込み、`\n` への置換はしない）。保存後、Claude 自身が `tmp/mureka_lyrics_prompt.txt` を Read で読み返し、上記【表記ルール】の文言（「歌詞は日本語のみで生成すること」で始まる一文）が含まれていることを確認してください。含まれていなければ追記してから次へ進みます。スクリプトはこのファイルの内容を無加工で API に送るため、この確認が表記ルール混入の最終ゲートになります。
