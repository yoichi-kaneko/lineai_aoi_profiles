...
```bash
cd {Base directory for this skill の絶対パス}
npx tsx scripts/download_file.ts <fileId>
```

例:

```bash
npx tsx scripts/download_file.ts 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgVE2upms
```
...
```json
{
  ...
  "savedPath": "/path/to/project/tmp/example.pdf"
}
```
