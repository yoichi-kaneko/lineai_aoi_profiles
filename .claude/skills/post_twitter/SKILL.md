...
- **投稿前のプラン確認・文字数上限の事前検証は行わない。** 本スキルは手動トリガーで使われ、プラン起因の失敗は稀なため、CLI は空本文以外の内容検証をせずそのまま投稿を試みる。投稿全文の上限は Twitter（X）の無料プラン以外で **25,000文字**（作法上の目安・超過時の扱いは [aoi_messaging.md](../../rules/aoi_messaging.md) の SNS 宛節を参照）。
- **投稿失敗時の扱い**（[aoi_constraints.md](../../rules/aoi_constraints.md) の汎用「1回リトライ→スキップ」より、本節を優先する）:
  - 原因が **メッセージ長超過**（25,000文字以内でも発生しうる）、または認証エラー・不正リクエストなど **同じ内容での再送で成功する見込みが薄いエラー** の場合は、再試行せず **処理を中断**し、ここまでの経過と失敗理由をユーザーへ説明して終了する。
