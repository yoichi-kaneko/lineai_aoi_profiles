...
打ち切りは「待つのをやめる」だけで、**サーバ側の書き込みをキャンセルするものではありません**。本スキルは書き込みのため、タイムアウトした場合は**そのまま再実行しないでください**。`get_firestore_docs` で当日分を読み直し、記録が無いことを確認できた場合に限り1回だけやり直します（[aoi_constraints.md の「タイムアウトの扱い」](../../rules/aoi_constraints.md) を正とします）。
