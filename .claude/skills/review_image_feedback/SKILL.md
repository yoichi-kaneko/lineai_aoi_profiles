...
- **`companions` の扱い（常時重み付けの軸）**: ルリ・蛍の登場可否は、ガイドライン セクション6・7で **`random_choice` の重み付き抽選**と定められています（ルリ `ruri:4 none:1`、蛍 `hotaru:1 none:4`。両者は独立抽選で、蛍は `scene_category` が `hike` のシーンでは抽選対象外＝登場しません）。設計上の期待分布は次のとおりです。
