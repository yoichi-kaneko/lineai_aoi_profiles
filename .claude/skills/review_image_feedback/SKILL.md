...
- **`codex_review` の扱い（分布軸ではなくクロス集計の材料）**: `codex_review`（[image_logs スキーマ](../../docs/image_log_schema.md)）は、プロンプトを外部レビュー（[codexレビュー](../../docs/codex_review.md)）に通した結果です。抽選軸ではないため**偏りフラグの対象にしません**。件数の内訳（`applied` / `not_applied` / `no_findings` / `skipped`）だけを控え、次の2点に使ってください。
  ...
  - **`skipped` が大半を占める場合**: レビュー工程が実質的に動いていない状態です。分布集計への影響はありませんが、その旨を報告に明記してください。
