...
- **偏りフラグは「是正命令」ではなく「要確認シグナル」**: 過剰フラグが立った値でも、ステップ2でその要素が繰り返し称賛されていれば「歓迎すべき偏り」であり、是正せず**むしろ伸ばす**候補とします（前提・思想の「偏りの二義性」）。称賛の裏付けがなく惰性で偏っているものだけを是正対象とします。いずれかは必ずステップ2の評価と突き合わせて判定してください（構図2軸 `shot_size` / `camera_direction` は例外。ステップ4の注釈を参照）。
...
- **彩りへの指摘**（構図・情景・衣装・時間帯の偏りや、もっと自由に等）→ 彩りの配分を調整する方向。調整には2方向があります。
  ...
  - **構図2軸（`shot_size` / `camera_direction`）の扱い（注釈）**:
    - この2軸は**当面、一様抽選を維持**します。「寄せる（偏り強化）」の修正案はこの2軸には作りません（ステップ5でも増頻案を出さない）。フィードバック受付によるガイドラインの変化量が短期間に大きくなりすぎ、不安定化するリスクを避けるためです。
    - ただし**長期的には、抽選制をある程度保ちつつ好ましい偏りも発生させる仕組みの確立を検討**しています（現時点では未確立）。
    - その仕組みが整う前に、ユーザーから構図2軸の傾向について**偏り強化の要望**が出ることも考えられます。その場合は、ガイドラインを今すぐ変えるのではなく、**put_todoist_task** で Todoist タスクとして記録し（タイトル例：`[構図軸要望] shot_size：バストアップの頻度を上げたい`）、将来の仕組み検討の材料として残してください。
    - なお「広げる」方向（惰性で過剰な値があるとき、欠落値が選ばれやすくなるよう語彙・説明を補強する等）は、一様抽選の枠内で従来どおり扱って構いません。
...
- どのセクション（核 or 彩りのどの項目）を、**どう変える**かを before/after で示します。例：
  ...
  - 彩りの調整（寄せる）：称賛された値を優先・増頻する旨を該当セクションに追記。例：ルリの称賛が続けば「6. ルリの登場指針」の登場頻度を引き上げる、夜の情景の称賛が続けば「8④」で夜の時間帯を優先候補にする。※構図2軸（`shot_size` / `camera_direction`）への増頻案は作らない（ステップ4の注釈参照。要望は Todoist へ記録）。
