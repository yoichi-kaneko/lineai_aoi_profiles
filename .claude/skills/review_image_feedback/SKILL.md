...
- **構図2軸の再抽選ログの扱い**: `shot_size_rerolled_from` / `shot_size_reroll_reason`（[image_logs スキーマ](../../docs/image_log_schema.md)）は、ガイドライン セクション8③の **抽選した2軸の成立チェック** でショットサイズを引き直した記録です（任意キーで、再抽選が無かった日は存在しません）。
  - **分布表は `shot_size` の採用値だけで数えます**。`shot_size_rerolled_from` の値を分布へ足し込まないでください（実際にその構図で描いていないため）。
  - `shot_size_rerolled_from` が付いたログの件数と、除外された値・カメラ方向の組み合わせは**別表として控えておきます**。特定の組み合わせ（例：`over_the_shoulder` × `knee_shot`）で再抽選が繰り返されているなら、語彙表の説明の補強や重みテーブルの見直しをステップ5の提案材料にしてください。
  - `shot_size_reroll_reason` が「2軸どうしが成立しない」という説明になっておらず、好みや描きやすさを理由にしていると読めるものが続く場合は、抽選制の趣旨（自己選択による偏りの打ち消し）が緩んでいるシグナルとして報告に含めます。
