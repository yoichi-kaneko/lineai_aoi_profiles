...
- **`companions` の扱い（シーン条件付きの重み付け軸）**: ルリ・蛍の登場可否は、ガイドライン セクション6・7で **`random_choice` の重み付き抽選**と定められています。`hike` 以外ではルリ（`ruri:4 none:1`）と蛍（`hotaru:1 none:4`）を独立に抽選し、`hike` では蛍を抽選対象外としてルリのみを抽選します。設計上の期待分布は次のとおりです。
  - `hike` 以外: ルリのみ 64% / 両方 16% / なし 16% / 蛍のみ 4%
  - `hike`: ルリのみ 80% / なし 20%

  したがって、上記の**過剰フラグ（40%）・欠落フラグは `companions` には適用しません**。`scene_category` が `hike` のログとそれ以外に分けたうえで、実測分布を設計値と突き合わせ、乖離の大きさで判断してください。設計値から大きく外れる場合（例：`hike` 以外が十分な件数あるのに蛍が1件も出ていない、ルリが全件に登場している）は、抽選が実施されていない可能性を含めて注記します。なお蛍単独（4%）は設計上もともと稀なため、出現0でも件数が少ないうちは偏りシグナルとして扱いません。
...
- **彩りへの指摘**（構図・情景・衣装・時間帯の偏りや、もっと自由に等）→ 彩りの配分を調整する方向。調整には2方向があります。
  ...
  - **`companions`（ルリ・蛍の登場可否）の扱い（注釈）**:
    - この軸も `random_choice` による**抽選制を維持**します。「寄せる」「広げる」のどちらも、ガイドライン **セクション6・7に記された重みの数値を増減する**形で提案してください（例：ルリの称賛が続く → `ruri:4 none:1` を `ruri:5 none:1` へ。蛍の登場が少なすぎるという評価が続く → `hotaru:1 none:4` を `hotaru:1 none:3` へ）。「ルリを毎回登場させる」のように抽選そのものを外す提案は行いません。
    - **重み0は使いません**（構図2軸と同じ）。ルリ・蛍のどちらも、引かれ得る状態を保ちます。
    - **山岳・自然シーンで蛍を抽選対象外とする扱いは、重みの調整対象ではありません**。これは蛍の活動領域がデジタル／都市空間に限られるという世界観上の制約（ガイドライン セクション7）であり、評価の集計で動かすものではありません。見直しが必要と判断した場合は、重みの調整ではなくキャラクター設定の変更としてユーザーに提起してください。
...
- どのセクション（核 or 彩りのどの項目）を、**どう変える**かを before/after で示します。例：
  ...
  - 彩りの調整（寄せる）：称賛された値を優先・増頻する旨を該当セクションに追記。例：夜の情景の称賛が続けば「8④」で夜の時間帯を優先候補にする。※構図2軸（`shot_size` / `camera_direction`）は、セクション8③の**重みテーブルの行追加・重みの増減・行の削除**という形で提案してください。`companions`（ルリ・蛍の登場可否）は、セクション6・7に記された**重みの数値の増減**という形で提案してください（例：ルリの称賛が続けば `ruri:4 none:1` を `ruri:5 none:1` へ）。いずれもステップ4の注釈を参照。before/after では、変更後の実効確率の目安も添えます。
