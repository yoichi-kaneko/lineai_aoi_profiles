...
- **意図的な重み付けの除外**: 上記の閾値は**一様抽選を前提とした目安**です。ガイドライン セクション8③の**重みテーブル**で重みを上げている値は、出現率が高くなるのが設計どおりのため、過剰フラグの対象外とし、必要なら「重み付けによる想定内の偏り」と注記するに留めます（重みテーブルに載っていない軸・値の偏りは従来どおり判定します）。ただし、想定を大きく超えて出ている場合は、重み値そのものが強すぎないかをステップ4・5で見直す材料にしてください。
...
- **彩りへの指摘**（構図・情景・衣装・時間帯の偏りや、もっと自由に等）→ 彩りの配分を調整する方向。調整には2方向があります。
  ...
  - **構図2軸（`shot_size` / `camera_direction`）の扱い（注釈）**:
    ...
    - **条件付きで設計する**: 重み付けは適用シーンを限定した形（例：「稜線・尾根を強調できる山行シーン」でのみ `wide_shot` の重みを上げる）を基本とします。条件を伴わない全面的な重み付けを行う場合は、その値が常態化して抽選制が形骸化しないよう、控えめな重みに留めてください。
