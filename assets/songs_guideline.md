...
これは厳密な制約ではなく、初回生成の成功率を高めるための目安です。シーンに応じて要素間で文字数を融通しても構いません（ジャンル名やタグの語が長くなる場合は、その分を instrument・description 側で吸収してください）。合計が930文字を大きく超える場合は、まず描写の重複や冗長な形容を見直し、それでも収まらない場合は後述の「目安を超えた場合の削り方」に従ってください。
...
1. **description の文を減らす**: 5要素の弧のうち [Intro] と [Outro] の描写を1文にまとめる、接続の言い回しを短くするなど。ただし **[Chorus] と [Vocal Style] は残してください**（サビでの感情の昂ぶりと、彩り軸の抽選結果を反映した声質は、Murekaの生成方向を決める要になるため）
2. **instrument の括弧内ディスクリプタを削る**: 2〜4語のディスクリプタを1〜2語へ削ります。ただし **楽器名そのものは削らないでください**（編成が変わり、スタイルパッケージの指定から外れてしまうため）

`genres` と `tags` は元々短く、Murekaがスタイル全体を推定する起点になるため、文字数調整のために削らないでください。

削る前に、描写の重複（instrument と description で同じ楽器へ同じ形容を重ねている、tags と genres で同じ方向性を繰り返している等）がないかを先に確認してください。重複の解消だけで収まることがあります。
