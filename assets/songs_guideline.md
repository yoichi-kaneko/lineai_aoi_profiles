...
以下の3軸は、パッケージ・抽選結果・週の空気に合わせて自由に選択して構いません。ただし「気づくと毎回同じ」になりやすい軸のため、**直近3曲と同じ選択が続いている軸は、必ず `random_choice` で抽選**してください（直近ログの確認は `modes/song.md` ステップ1で取得済みの `song_logs` で足ります）。とりわけ「ブリッジの有無」は「なし」が選ばれにくく、反復の心地よさで聴かせる曲が生まれない原因になります。
...
- **テンポ感・明暗** → `tags` と `description` の全体トーン。テンポ感は抽選結果に対応する語（`Slow Ballad` / `Mid-tempo` / `Uptempo` / `Dynamic`）を**必ず1つ `tags` に含める**（`song_logs` に残り、後のレビューで曲調の連続を検証する手がかりになる）
